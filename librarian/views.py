...
##############################################################################
##        pk=int(request.POST['pk'])
##        pap=Paper.objects.get(pk=pk)
##        
##        cit_list=Raw_cit.objects.filter(last_name=pap.first_au.last_name).filter(year=pap.pubYear)
##        try:
##                j=pap.journal
##                cit_list=cit_list.filter(jour=j)
##        except:
##                pass
##        
##        cit_list=list(cit_list)
##        if len(cit_list)>1:
##                for xcit in cit_list:
##                        if xcit.vol and xcit.vol!=pap.volume:
##                                cit_list.remove(xcit)
##
##        if len(cit_list)==1:
##                cit=cit_list[0]
##                wp=cit.get_paper()
##                if wp!=pap:
##                        citations=Citation.objects.filter(cited_paper=wp)
##                        for xcit in citations:
##                                xcit.cited_paper=pap
##                                xcit.save()
##                                
##                        pap.raw_cit=cit
##                        pap.save()
...
##        cit.save()
##        p=cit.get_paper()
##        p.journal=cited_jour
##        p.save()

        
        

def correct_journals(request,ran):
        ran=int(ran)
        all_paps=Paper.objects.all()[ran*2000:(ran+1)*2000]
        for pap in all_paps:
                if pap.title and not pap.journal:
                        if not pap.raw_cit:
                                raw_cit=None
                                if pap.doi:
                                        raw_cit=Raw_cit.objects.filter(doi=pap.doi)
                                if not raw_cit:
                                        raw_cit=Raw_cit.objects.filter(last_name=pap.frist_au.last_name)
